...
function checkAnswer(answer) {
    ...
    if (yourAnswer == answer) {
        section.innerHTML = '<p>Oui! Bravo!</p><button type="button" class="revision basic-button" onclick="ajaxCallAsynch(' + currentNum + ')">Re-teste moi!</button>';
...
function checkMultipleAnswers(answer) {
    ...
    for (var i = 0; i < correctAnswers.length - 1; i++) {
        if (yourAnswer[i].value == correctAnswers[i]) {
            score += 1;
            console.log(yourAnswer);
            yourAnswer[i].style.backgroundColor = "#8aa751";
    ...
    section.innerHTML = '<p>Votre score est de : ' + score + '/5</p><button type="button" class="revision basic-button" onclick="ajaxCallAsynchSuper(' + "'" + currentNum + "'" + ')">Re-teste moi!</button>';
